...
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
read z11x z11y z13x z13y z16x z16y leftx lefty rightx righty <<EOF
$(python3 - <<'PY'
import math
def tile(lon, lat, z):
 return int((lon + 180) / 360 * 2**z), int((1 - math.asinh(math.tan(math.radians(lat))) / math.pi) / 2 * 2**z)
for z in (11, 13, 16): print(*tile(10.8658, 60.0312, z), end=' ')
print(*tile(10.8450, 60.0312, 16), *tile(10.8850, 60.0312, 16))
PY
)
EOF
for zxy in "11 $z11x $z11y" "13 $z13x $z13y" "16 $z16x $z16y" "16 $leftx $lefty" "16 $rightx $righty"; do
  set -- $zxy
...
docker run --rm -v "$tmp:/data:ro" \
  -e Z11="11-$z11x-$z11y" -e Z13="13-$z13x-$z13y" -e Z16="16-$z16x-$z16y" \
...
z11, z13, z16 = (features(os.environ[name]) for name in ("Z11", "Z13", "Z16"))
...
print(f"z11={len(z11)} z13={len(z13)} z16={len(z16)}: schema, zoom rules, and boundary ID ok")
PY
'
